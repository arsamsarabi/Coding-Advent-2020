...
export const getParsedAnswers = (): any => {
  const parsedGroups = RAW_ANSWERS.split('\n\n')
  const parsedPeople = parsedGroups.map((group: string) => group.split('\n'))
  const parsedPerson = parsedPeople.map((group: string[]) => {
    return group.map((person: string) => person.split(''))
  })
